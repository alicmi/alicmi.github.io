...
const ContentPanel: React.FC<ContentPanelProps> = ({ activeProject, activeTopic, showCV, onLinkClick, onShowCV }) => {
  ...
  if (showCV) {
    return (
      <div 
        ...
        className="h-full w-full p-8 lg:p-12 overflow-y-auto backdrop-blur-md bg-gradient-to-r from-[#114552]/0 via-[#0C323D]/85 to-[#0C323D]/95 text-white z-10 custom-scrollbar relative"
      >
         
         {/* Title area */}
         <div className="relative z-20 mb-8 lg:mb-12">
             <h1 className="text-xl lg:text-2xl font-light text-[#FFB800] tracking-widest uppercase m-0">
               Curriculum Vitae
             </h1>
         </div>
         ...
         <div className="relative">
            <div className="space-y-12 relative z-10">
                 ...
                 <div className="relative group">
                     <h2 className="text-base font-bold font-mono text-white mb-2 group-hover:text-[#FFB800] transition-colors">High school</h2>
                     <p className="text-[10px] lg:text-xs font-mono text-[#FFB800]/70 mb-3 tracking-wider">2013-2021</p>
                     <ul className="list-none space-y-2 text-xs lg:text-sm font-mono opacity-90 border-l border-[#FFB800]/30 pl-4">
                 ...
                 <div className="relative group pt-4">
                     <h2 className="text-base font-bold font-mono text-white mb-2 group-hover:text-[#FFB800] transition-colors">Internships and personal volunteering</h2>
                     {/* Removed date range as requested */}
                     <ul className="list-none space-y-2 text-xs lg:text-sm font-mono opacity-90 border-l border-[#FFB800]/30 pl-4 mt-3">
                         <li className="flex items-start">
                         ...
                         <li className="flex items-start">
                             <span className="mr-2 text-[#FFB800] opacity-70">•</span> Six-month internship at creative industry lobby “creative.nrw”, BOROS
                         </li>
                         <li className="flex items-start">
                 ...
                 {/* Section 3: Education */}
                 <div className="relative group pt-4">
                     <h2 className="text-base font-bold font-mono text-white mb-2 group-hover:text-[#FFB800] transition-colors">Academic further education</h2>
                     
                     <div className="mb-6">
                        <p className="text-[10px] lg:text-xs font-mono text-[#FFB800]/70 mb-2 tracking-wider">2022-2024</p>
                        <ul className="list-none space-y-2 text-xs lg:text-sm font-mono opacity-90 border-l border-[#FFB800]/30 pl-4">
                            <li className="flex items-start">
                     ...
                     <div>
                        <p className="text-[10px] lg:text-xs font-mono text-[#FFB800]/70 mb-2 tracking-wider">2024-2029</p>
                        <ul className="list-none space-y-2 text-xs lg:text-sm font-mono opacity-90 border-l border-[#FFB800]/30 pl-4">
                            <li className="flex items-start">
  ...
  return (
    <div 
      onClick={(e) => e.stopPropagation()}
      className="h-full flex flex-col p-12 lg:p-16 overflow-y-auto backdrop-blur-md bg-gradient-to-r from-[#114552]/0 via-[#0C323D]/85 to-[#0C323D]/95 text-white z-10 transition-colors duration-500 custom-scrollbar"
    >
      ...
      {activeProject ? (
        <div className="animate-fade-in-up flex flex-col pb-10">
           ...
           <div className="sticky top-[-3rem] lg:top-[-4rem] pt-4 pb-4 z-20 backdrop-blur-xl bg-[#0C323D]/80 mb-8 border-b border-[#FFB800]/20 -mx-4 px-4 shadow-sm">
             <h1 className="text-xl lg:text-2xl font-light text-[#FFB800] tracking-widest uppercase m-0">
               {activeProject.label}
             </h1>
             {/* Subtitle / Meta Info */}
           ...
           {getDirectImageUrl(activeProject.img) && (
               <div className="mb-8 shrink-0">
                   <div className="w-full h-64 lg:h-96 rounded-sm overflow-hidden bg-black/20 relative">
                      <img 
                        key={getMediaUrl(activeProject.img)} 
                        src={getDirectImageUrl(activeProject.img)} 
                        alt={activeProject.label} 
                        className={`w-full h-full object-cover grayscale-[20%] hover:grayscale-0 transition-all duration-700 hover:scale-105 ${activeProject.headerImagePosition || 'object-center'}`}
           ...
           {/* 3. Text Content */}
           <div className="font-mono text-xs lg:text-sm leading-relaxed opacity-90 space-y-3 text-justify mb-8">
           ...
           {activeProject.gallery && activeProject.gallery.map((item, idx) => (
             <div key={`img-${idx}`} className="w-full mt-4 shrink-0">
                 <div className="rounded-sm overflow-hidden bg-black/20 relative">
                    <img 
        ...
        <div className="animate-fade-in-up flex flex-col pb-10">
            <h1 className="text-xl lg:text-2xl font-light mb-6 text-[#FFB800] tracking-widest uppercase shrink-0">
              {activeTopic}
            </h1>
            
            {/* Topic Description (if available) */}
            {TOPIC_DESCRIPTIONS[activeTopic] && (
                <div className="font-mono text-xs lg:text-sm leading-relaxed opacity-90 text-justify mb-8 border-l-2 border-[#FFB800] pl-4 py-1">
                    {TOPIC_DESCRIPTIONS[activeTopic]}
                </div>
            )}
            ...
            {TOPIC_GALLERIES[activeTopic] && TOPIC_GALLERIES[activeTopic].length > 0 && (
                 <div className="mb-4 space-y-4">
                    {TOPIC_GALLERIES[activeTopic].map((imgUrl, idx) => (
                         <div key={`topic-img-${idx}`} className="w-full rounded-sm overflow-hidden bg-black/20 shrink-0 relative">
                            <img 
                ...
                {topicProjects.length > 0 ? (
                    topicProjects.map(p => (
                        <div 
                            ...
                            className="group cursor-pointer bg-black/10 hover:bg-[#FFB800]/10 p-4 rounded-sm border-l-2 border-transparent hover:border-[#FFB800] transition-all duration-300 focus:outline-none focus:ring-1 focus:ring-[#FFB800]/50"
                        >
        ...
        <div className="flex flex-col justify-center h-full animate-fade-in relative">
           <div className="font-mono text-xs lg:text-sm leading-relaxed opacity-90 space-y-4 lg:space-y-6">
             ...
             <div className="mt-8 pt-6 border-t border-[#FFB800]/30">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    if(onShowCV) onShowCV();
                  }}
                  className="text-sm font-light text-[#FFB800] hover:text-white transition-colors tracking-widest uppercase border-b border-transparent hover:border-[#FFB800] pb-0.5"
                >
                  Curriculum Vitae
                </button>
             </div>
